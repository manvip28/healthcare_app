      ...
      build: (pw.Context context) {
        return pw.Column(
          ...
          children: [
            ...
            pw.Text('Age: ${patient.age} years',style: pw.TextStyle(fontWeight: pw.FontWeight.bold)),
            pw.Text('Diagnosis: ${patient.diagnosis}',style: pw.TextStyle(fontWeight: pw.FontWeight.bold)),
            pw.Text('Gender: ${patient.gender}', style: pw.TextStyle(fontWeight: pw.FontWeight.bold)),
            pw.SizedBox(height: 20),

            // Adding the optional fields
            if (patient.medicalHistory != null && patient.medicalHistory!.isNotEmpty)
              pw.Text('Medical History: ${patient.medicalHistory}', style: pw.TextStyle(fontWeight: pw.FontWeight.bold)),
            if (patient.allergies != null && patient.allergies!.isNotEmpty)
              pw.Text('Allergies: ${patient.allergies}', style: pw.TextStyle(fontWeight: pw.FontWeight.bold)),
            if (patient.familyMedicalHistory != null && patient.familyMedicalHistory!.isNotEmpty)
              pw.Text('Family Medical History: ${patient.familyMedicalHistory}', style: pw.TextStyle(fontWeight: pw.FontWeight.bold)),
            if (patient.medications != null && patient.medications!.isNotEmpty)
              pw.Text('Medications: ${patient.medications}', style: pw.TextStyle(fontWeight: pw.FontWeight.bold)),
            if (patient.ongoingTreatments != null && patient.ongoingTreatments!.isNotEmpty)
              pw.Text('Ongoing Treatments: ${patient.ongoingTreatments}', style: pw.TextStyle(fontWeight: pw.FontWeight.bold)),
...
pw.Widget _buildVitalRecordForPdf(Vitals vital) {
  ...
  final systolic = bloodPressureParts.isNotEmpty ? bloodPressureParts[0] : 'N/A';
  final diastolic = bloodPressureParts.length > 1 ? bloodPressureParts[1] : 'N/A';
  final date = vital.dateTime.toIso8601String().split('T')[0]; // Use dateTime getter
